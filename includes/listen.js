...
(async function () {
  ...
  return (event) => {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`আপনার গ্রুপ  approved নেই   approved এর জন্য আমার বস কে ${prefix}request দেন\n\n𝐀𝐒𝐈𝐅 𝐱𝟔𝟗\n 
Owner : 𝐀𝐒𝐈𝐅 𝐱𝟔𝟗\n

 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝 : https://www.facebook.com/4S1F.403\n

আপনার গ্রুপ এ যদি না হয়ে থাকে তো আপনি গ্রুপ  admin কে  add করতে পারেন আপনার গ্রুপ এ 😇`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
